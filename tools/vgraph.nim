...
iterator importedModules(path: string): string =
  ...
  ##
  ## The last component is what layer names are matched against, so a module
  ## whose basename equals a layer name counts as that layer wherever it sits
  ## — `anything/probe` reads as the `probe` layer. Name a module distinctly
  ## from any layer and the question does not arise.
...
iterator requiredPackages(path: string): string =
  ## Package name of every spec on every `requires` line.
  ##
  ## Every quoted span, not just the first: `requires "a", "b"` is one line
  ## naming two packages, and a check that read only the first would pass a
  ## back-edge hidden in the second — the one failure a dependency checker
  ## must not have.
  for raw in readFile(path).splitLines:
    let line = raw.strip
...
proc main() =
  ## Check every module under `src` against the layer order, and every `Uni*`
  ## in `requires` against the declared engines. Names each violation and
  ## exits non-zero, or reports what it checked and exits clean.
