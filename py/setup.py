...
"""Build unimovie._core, a Cython extension over the UniMovie C ABI.
...
PKG_DIR = os.path.join(HERE, "unimovie")
VENDOR_DIR = os.path.join(HERE, "_nimsrc")
NIMBLE_FILE = "UniMovie.nimble"
...
if sys.platform == "win32":
    LIB_NAME, BUNDLED = "UniMovie.lib", False
    LINK_ARGS, NIMBLE_TASK = [], "clibMsvc"
elif sys.platform == "darwin":
    LIB_NAME, BUNDLED = "libUniMovie.dylib", True
    LINK_ARGS, NIMBLE_TASK = ["-Wl,-rpath,@loader_path"], "clib"
else:
    LIB_NAME, BUNDLED = "libUniMovie.so", True
...
def nim_project_dir():
    """Where UniMovie.nimble lives: the real repo root in a normal
...
def ensure_lib_built():
    ...
    try:
        ...
        raise SystemExit(
            "setup.py: `nimble` not found on PATH. Building unimovie from "
...
# The sdist ships the pre-transpiled unimovie/_core.c, not the .pyx (Cython
...
# needing no Cython.
pyx = os.path.join("unimovie", "_core.pyx")
ext = Extension(
    "unimovie._core",
    sources=[pyx if os.path.exists(os.path.join(HERE, pyx)) else os.path.join("unimovie", "_core.c")],
...
setup(
    ...
    package_data={"unimovie": [LIB_NAME] if BUNDLED else []},
    exclude_package_data={"unimovie": ["_core.c"]},
    zip_safe=False,
)
